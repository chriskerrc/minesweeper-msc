...
board moore_count(unsigned width, unsigned height, board b) 
{
int w = (int) width;
int h = (int) height;
int cnt= 0;
for(int row = 0; row < h; row++){    
         for(int col = 0; col < w; col++){ 
            if(b.grid[row][col] == UNK){
               int row_unk = row;
               int col_unk = col;
                    if(row>0){   // can I simplify this bounds checking, given that the problem was with the count, not the bounds checking? 
